...
public class DepenseQueryService extends QueryService<Depense> {
    ...
    public Specification<Depense> createSpecification(String critereTransversal) {
        log.info("Création de la spécification à partir du critère transversal: {}", critereTransversal);
        Specification<Depense> specification = null;
        if (critereTransversal != null) {
      /*      StringFilter filter = new StringFilter();
            filter.setContains(critereTransversal);
            specification = Specification
                .where(buildStringSpecification(filter, Depense_.nom))
